...
5. Fill out the necessary information in the docker.env file:
   ...
   * **LOG_ENV**: Optional environment tag for categorizing logs and metrics (e.g., `staging`, `production`, `dev`). This tag will be added to all logs and metrics sent to Datadog as `env:<value>`
6. Execute `docker run` as shown below.
   ...
   ```bash
    ...
    docker run -it --user root --name jfrog-fluentd-datadog-rt -v $JFROG_HOME/artifactory/var/:/var/opt/jfrog/artifactory --env-file docker.env jfrog/fluentd-datadog-rt
   ```
